...
export type ContratoFormData = {
  ...
  duracaoHoras: string;
  duracaoCerimonia: string;
  duracaoRecepcao: string;
...
export const MERGE_FIELDS: MergeField[] = [
  {
    ...
    format: (d) => {
      if (d.dataEventoTexto) return escapeHtml(d.dataEventoTexto);
      if (d.dataEventoFim) {
        return `${formatLongDate(d.dataEvento)} a ${formatLongDate(d.dataEventoFim)}`;
      }
      return formatLongDate(d.dataEvento);
    },
  },
  {
    key: "horaEvento",
  ...
  {
    key: "duracaoCerimonia",
    label: "Duração da cerimônia (horas)",
    format: (d) => escapeHtml(d.duracaoCerimonia) || "______",
  },
  {
    key: "duracaoRecepcao",
    label: "Duração da recepção (horas)",
    format: (d) => escapeHtml(d.duracaoRecepcao) || "______",
  },
  {
    key: "quantidadeEquipe",
    label: "Quantidade de pessoas da equipe",
    format: (d) => escapeHtml(d.quantidadeEquipe) || "______",
  },
